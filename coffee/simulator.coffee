...
View.SizeCanvas = () ->
  ...
  if w > h
    ...
    View.body_boundary = (2 * View.body_size) ** 2
...
View.Update = () ->
  ...
  if elapse < 17
    View.render_id = window.setTimeout callback, 17 - elapse
  else
...
Engine.ForceEvaluation = () ->
  ...
  for i in [1..Engine.n - 1]
    for j in [0..i - 1]
      ...
      # particles sling themselves past one another. (Fixed timestep size is too big to
      # handle perige properly.)
      if dist_sq > 100
        force = Engine.G / dist_sq
      else

        if Engine.collide and dist_sq < View.body_boundary
...
Engine.ApplyConstraints = () ->
  ...
  if Engine.collide and Engine.collide_i.length > 0
    ...
    separation = 2 * View.body_size + 0.5
    ...
    if Engine.sticky
      for k in [0..Engine.collide_i.length - 1]
        ...
        #Engine.pos_x[i] = Engine.pos_x[j] + separation * x * dist_inverse
        #Engine.pos_y[i] = Engine.pos_y[j] + separation * y * dist_inverse
      ...
      for k in [0..Engine.collide_i.length - 1]
        ...
        vel_ij_x = Engine.vel_x[i] - Engine.vel_x[j]
        vel_ij_y = Engine.vel_y[i] - Engine.vel_y[j]

        if vel_ij_x > 0 and vel_ij_y > 0
          # The particles are moving apart.  Don't do anything to them.
          continue
        else if vel_ij_x > 0 and vel_ij_y < 0 and Math.abs(vel_ij_x) > Math.abs(vel_ij_y)
          # The particles are moving apart.  Don't do anything to them.
          continue
        else if vel_ij_x < 0 and vel_ij_y > 0 and Math.abs(vel_ij_x) < Math.abs(vel_ij_y)
          # The particles are moving apart.  Don't do anything to them.
        ...
        normal_x = x * dist_inverse * 0.5
        normal_y = y * dist_inverse * 0.5

        Engine.vel_x[i] -= normal_x * vel_ij_x
        Engine.vel_x[j] += normal_x * vel_ij_x

        Engine.vel_y[i] -= normal_y * vel_ij_y
        Engine.vel_y[j] += normal_y * vel_ij_y
  ...
  for i in [0..Engine.n - 1]
    ...
    if Engine.vel_x[i] < -speed
      Engine.vel_x[i] = -speed
    else if Engine.vel_x[i] > speed
      Engine.vel_x[i] = speed

    if Engine.vel_y[i] < -speed
      Engine.vel_y[i] = -speed
    else if Engine.vel_y[i] > speed
      Engine.vel_y[i] = speed
...
Engine.ChangeCollide = () ->
  box = document.getElementById('collide_control')

  if box.checked
    Engine.collide = true
  else
    Engine.collide = false
